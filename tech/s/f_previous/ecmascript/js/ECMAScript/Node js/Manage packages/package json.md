...
I also want to know about initial value
...
- `package.json`
    ...
    NAME
       ...
       name
           ...
           •   The name must be less than or equal to 214 characters. This
           ...
           •   Don't put "js" or "node" in the name. It's assumed that it's js,
           ...
           •   You may want to check the npm registry to see if there's something
       ...
       files
           ...
           •   package-lock.json (use npm-shrinkwrap.json ⟨/configuring-npm/npm-
       ...
       dependencies
           ...
           •   tag A specific version tagged and published as tag See npm help
               dist-tag
...
```json
"author": "Yu Nai <yunaimatsu@gmail.com> (https://yunaimatsu.io)",
```
...
外部からパッケージを使う時のエントリーポイント。パッケージが読み込まれた時、このファイルをまず最初に実行する。
...
|  | Version | Change content |  | Backward compatibility |  |
| --- | --- | --- | --- | --- | --- |
| **`x`** | **Major version** | Major changes |  | not guaranteed | `y`.`z` will be reset to `0` |
| **`y`**  | **Minor version** | New function | `~`: 最新の`y` | guaranteed | `z` will be reset to `0` |
| **`z`** | **Patch version** | Bug fix | `^`: 最新の`z` |  |  |
- How to designate the version
...
Mainly used when it builds production code.

- Module bandler
    
    [Browserslist](https://github.com/browserslist)
    
    - Webpack
        
        サポート対象のブラウザの設定があればそのブラウザで動作するように後方互換のコードを自動的に注入。CSSのコードに後方互換を持たせるAutoprefixerなどもこの設定。
        
- Examples of `package.json`
    
    [https://github.com/makeplane/plane/blob/preview/package.json](https://github.com/makeplane/plane/blob/preview/package.json)
    
    [https://github.com/jitsi/jitsi-meet/blob/master/package.json](https://github.com/jitsi/jitsi-meet/blob/master/package.json)
    
    [https://github.com/AppFlowy-IO/AppFlowy-Website/blob/main/package.json](https://github.com/AppFlowy-IO/AppFlowy-Website/blob/main/package.json)
